...
const Feature = (props) => {
  return (
    <div className={`flex flex-row p-6 rounded-[20px] ${props.index !== props.features.length - 1 ? "mb-6" : "mb-0"} feature-card`}>
      <div className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter} bg-dimBlue`}>
        <img src={props.icon} alt="icon" className='w-[50%] h-[50%] object-contain'/>
      </div>
   ...
   <section id='features' className={layout.section}>
    ...
    <div className={`${layout.sectionImg} flex-col`}>
      {features.map( (feature, index) => (
        <Feature key={feature.id} {...feature} features={features} index={index}/>
      ))}
    </div>
   </section>
  )
